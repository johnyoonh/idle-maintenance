...
- On systems with `terminal-notifier`, clicking **Show** opens `resource-monitor-history.jsonl` in VS Code when available, then falls back to the default macOS text editor. The AppleScript fallback is used only when `terminal-notifier` is missing or fails before delivery (such as an execution error); timeouts and ambiguous delivery attempts never fall back to AppleScript, guaranteeing at most one visible alert per incident.
